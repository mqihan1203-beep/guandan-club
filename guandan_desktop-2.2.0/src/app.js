...
function render() {
  ...
  if (state.status==='tribute') renderTributeCenter(state);
    ...
    const lp=$('#lastPlay'); lp.innerHTML=''; state.lastPlay.cards.forEach((c,i)=>lp.appendChild(renderCard(c,false,i,false)));
  } else {
  ...
  if (isSpectator()) setStatus('正在观战。可以在右侧聊天；出现空位时可在联机大厅补位。');
    ...
    $('#newGameBtn').textContent='贡还牌中'; $('#newGameBtn').disabled=true;
    ...
    $('#newGameBtn').textContent=online?'本局进行中':'重新开局'; $('#newGameBtn').disabled=online;
  ...
  try{
    ...
    while(localGame&&guard++<90){
      if(localGame.status==='tribute'){
        ...
        await new Promise(r=>setTimeout(r,220)); const card=AI.chooseReturnCard(localGame,botEntry.receiver); if(!card)break; core.returnTribute(localGame,botEntry.receiver,card.id); render(); continue;
      }
      if(localGame.status!=='playing'||localGame.turn===0)break;
      await new Promise(r=>setTimeout(r,250)); const seat=localGame.turn; const move=AI.chooseMove(localGame,seat,$('#difficultySelect').value);
      if(move.action==='play')core.playCards(localGame,seat,move.cards.map(c=>c.id)); else core.passTurn(localGame,seat); render();
    }
...
function connect(url, mode=networkMode){
  closeSocket(false); url=url.trim(); if(!/^wss?:\/\//i.test(url))url='ws://'+url;
  try{socket=new WebSocket(url);}catch(e){return setLobbyInfo(`连接地址无效：${e.message}`);}
  networkMode=mode; lastServerUrl=url; storage.setItem('guandan:lastServerUrl',url); storage.setItem('guandan:lastNetworkMode',mode);
  $('#roomState').textContent='正在连接…'; connected=false; renderLobby();
  socket.onopen=()=>{connected=true;const name=clampName($('#playerNameInput').value);storage.setItem('guandan:nickname',name);send({type:'join',name,seat:desiredSeat,reconnectToken});};
  socket.onmessage=e=>{
    let msg;try{msg=JSON.parse(e.data);}catch{return;}
    if(msg.type==='hello'){currentRoomCode=msg.roomCode||'';$('#roomState').textContent=`已连接 ${msg.roomName} · 房间码 ${msg.roomCode}`;}
    if(msg.type==='room'){
      online=true;connected=true;mySeat=msg.you.seat;desiredSeat=(msg.you.seat===null||msg.you.seat===undefined)?-1:msg.you.seat;myClientId=msg.you.id;isHost=!!msg.you.host;currentRoomCode=msg.room.roomCode||currentRoomCode;roomSnapshot=msg.room;
    ...
    if(msg.type==='state'){online=true;connected=true;mySeat=msg.you.seat;myClientId=msg.you.id;isHost=!!msg.you.host;onlineState=msg.state;selected.clear();render();renderLobby();maybeAutoOnline();}
    ...
    if(msg.type==='kicked'){setStatus(`你已被房主移出房间${msg.reason?`：${msg.reason}`:''}`,true);$('#roomState').textContent='已被移出房间';}
    if(msg.type==='error'){setStatus(msg.message,true);$('#roomState').textContent=msg.message;}
  };
  socket.onclose=()=>{connected=false;$('#roomState').textContent='连接已断开，可点击“断线重连”';$('#reconnectBtn').classList.remove('hidden');renderLobby();if(online)setStatus('联机连接已断开，牌桌会保留当前画面',true);};
  socket.onerror=()=>{$('#roomState').textContent='连接失败，请检查地址、端口和防火墙';};
}
...
function renderDiscovered(){
  ...
  for(const r of active){const div=document.createElement('div');div.className='room-item';const span=document.createElement('span');span.innerHTML=`<b>${esc(r.roomName)}</b><br><small>${esc(r.host)}:${esc(r.port)} · ${esc(r.roomCode)}</small>`;const btn=document.createElement('button');btn.textContent='加入';btn.onclick=()=>{setNetworkMode('lan');const url=`ws://${r.host}:${r.port}`;$('#lanServerUrlInput').value=url;connect(url,'lan');};div.append(span,btn);box.appendChild(div);}
}
function renderLobby(){
  $('#lobbyNetworkBadge').textContent=networkMode==='lan'?'局域网':'公网';
  $('#lobbyRoomCode').textContent=roomSnapshot?.roomCode||currentRoomCode||'------';
  $('#lobbyRoomTitle').textContent=roomSnapshot?.roomName||'在这里，等你入座';
  $('#lobbyReadyBadge').textContent=connected?(isSpectator()?'替补观战':(roomSnapshot?.seats?.[mySeat]?.ready?'已准备':'等待准备')):(online?'已断线':'未连接');
  $('#reconnectBtn').classList.toggle('hidden',connected||!lastServerUrl);
  const seats=roomSnapshot?.seats||[null,null,null,null];
  $$('.lobby-seat').forEach((card,i)=>{
    const s=seats[i];card.classList.toggle('occupied',!!s);card.classList.toggle('me',!!s&&s.id===myClientId);card.classList.toggle('selected',!connected&&desiredSeat===i);
    const team=i%2===0?'绿队':'蓝队';
    if(s){
      const ready=s.ready?'已准备':'未准备';const host=s.host?' · 房主':'';const kick=isHost&&s.id!==myClientId?`<span class="kick-hit" data-kick="${esc(s.id)}" title="踢出">×</span>`:'';
      card.innerHTML=`<span>${i+1} 号位 · ${team}${kick}</span><b>${esc(s.name)}</b><small>${ready}${host}</small>`;
    }else card.innerHTML=`<span>${i+1} 号位 · ${team}</span><b>＋ 点击入座</b><small>${connected&&roomSnapshot?.gameStarted?'空位 · 可替补':'空位'}</small>`;
  });
  ...
  const mine=mySeat!==null?seats[mySeat]:null;$('#readyBtn').disabled=!connected||isSpectator()||!!roomSnapshot?.gameStarted;$('#readyBtn').textContent=mine?.ready?'取消准备':'准备';
  $('#startOnlineBtn').style.display=isHost?'':'none';$('#startOnlineBtn').disabled=!connected||!isHost||!(roomSnapshot?.canStart??false)||!!roomSnapshot?.gameStarted;
  $('#toBenchBtn').disabled=!connected||isSpectator();
  if(roomSnapshot){const seatText=seats.map((s,i)=>`${i+1}号位：${s?s.name:'AI/空位'}`).join('　');$('#roomState').textContent=`${roomSnapshot.roomName} · ${seatText}${bench.length?` · 替补 ${bench.length}/2`:''}`;}
}
...
async function handleMainNewGame(){const state=currentState();if(online){if(!state)return;if(!isHost)return setStatus('只有房主可以开始下一局',true);if(state.status==='round-finished'){hideResult();send({type:'nextRound'});}else if(state.status==='match-finished'){hideResult();send({type:'start',levelRank:'2',difficulty:$('#difficultySelect').value});}return;}if(localGame?.status==='round-finished')return advanceLocalRound();hideResult();await newLocalGame();}
...
$('#manualSortBtn').onclick=()=>setManualMode(true);$('#autoSortBtn').onclick=()=>setManualMode(false);
...
$('#reconnectBtn').onclick=()=>{if(lastServerUrl)connect(lastServerUrl,storage.getItem('guandan:lastNetworkMode')||networkMode);};
...
$('#hostLanBtn').onclick=async()=>{
  ...
  try{const info=await window.desktopAPI.startLanServer({roomName:$('#roomNameInput').value,port:Number($('#portInput').value)});const ips=(info.addresses||[]).map(x=>typeof x==='string'?x:x.address);setLobbyInfo(`房间已创建 · 房间码 ${info.roomCode}\n局域网地址：${ips.map(ip=>`ws://${ip}:${info.port}`).join(' / ')}`);const url=`ws://127.0.0.1:${info.port}`;$('#lanServerUrlInput').value=url;connect(url,'lan');}catch(e){setLobbyInfo(`创建失败：${e.message}`);}
};
$('#connectLanBtn').onclick=()=>{const u=$('#lanServerUrlInput').value.trim();if(!u)return setLobbyInfo('请输入局域网 ws://IP:端口');connect(u,'lan');};
$('#connectPublicBtn').onclick=()=>{const u=$('#publicServerUrlInput').value.trim();if(!u)return $('#roomState').textContent='请输入公网 WebSocket 地址';connect(u,'public');};
$('#startOnlineBtn').onclick=()=>{if(!socket)return setLobbyInfo('请先连接房间');if(!isHost)return setLobbyInfo('只有房主可以开始');send({type:'start',levelRank:$('#levelRankSelect').value,difficulty:$('#difficultySelect').value});$('#lobbyDialog').close();};
...
if(lastServerUrl){$('#lanServerUrlInput').value=lastServerUrl;$('#publicServerUrlInput').value=lastServerUrl;}

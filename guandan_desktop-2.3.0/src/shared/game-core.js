(function (root, factory) {
  ...
  function createInitialGame({
    ...
    return {
      ...
      leader: dealer,
      lastPlay: null,
      tablePlays: [null,null,null,null],
      passCount: 0,
  ...
  function playCards(state, seat, cardIds) {
    ...
    if (!Array.isArray(state.tablePlays)) state.tablePlays = [null,null,null,null];
    if (!state.lastPlay) state.tablePlays = [null,null,null,null];
    state.tablePlays[seat] = { seat, cards, pattern };
    state.lastPlay = { seat, cards, pattern };
    state.leader = seat;
    state.passCount = 0;
    state.history.push({ type: 'play', seat, cards: cards.map(cardText), cardData: cards.map(c=>({rank:c.rank,suit:c.suit})), pattern: pattern.name, at: Date.now() });
  ...
  function publicState(state, seat = null) {
    ...
    return {
      ...
      tablePlays: (state.tablePlays || [null,null,null,null]).map(p => p ? ({seat:p.seat,cards:p.cards,pattern:p.pattern}) : null),

  ...
  if (selectable) {
    ...
    div.oncontextmenu = e => {
      ...
      if(!selected.has(c.id)){
        // 右键未选中的牌时，按“快速出这张”处理；已有合法多选则右键选中牌可直接出整组。
...
function connect(url, mode=networkMode, roomCode=lastRoomCode){
  ...
  socket.onmessage=e=>{
    ...
    if(msg.type==='hello'){
      ...
      if(serverCode!==cleanCode){setRoomMessage('房间码与目标房间不匹配，请重新输入',true);closeSocket(false);return;}
  ...
  socket.onerror=()=>{setRoomMessage(mode==='lan'?'连接失败：请确认同一局域网、防火墙和房间码':'连接失败：请确认房主公网端口可访问、房间码正确',true);};
}
...
function joinRoomByCode(rawCode,mode){
  ...
  try{
    const decoded=networkCode.decodeRoomCode(clean);
  ...
  }catch(e){setRoomMessage(`房间码无效：${e.message}`,true);}
}

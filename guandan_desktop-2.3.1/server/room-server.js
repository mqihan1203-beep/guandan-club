...
async function createRoomServer({port=37821,roomName='掼蛋房间',advertise=false,roomCode:requestedRoomCode=''}={}) {
  const roomCode=networkCode.normalizeRoomCode(requestedRoomCode)||randomCode();
  ...
  function joinClient(c,msg){
    cleanupSessions();
    const suppliedRoomCode=networkCode.normalizeRoomCode(msg.roomCode||'');
    if(!suppliedRoomCode||suppliedRoomCode!==networkCode.normalizeRoomCode(roomCode))return{ok:false,error:'房间码不正确'};
  ...
  async function runBots(){
    ...
    try{
      ...
      while(game&&guard++<100){
        ...
        let result=move.action==='play'&&Array.isArray(move.cards)&&move.cards.length
          ? core.playCards(game,seat,move.cards.map(c=>c.id))
          : core.passTurn(game,seat);
        if(!result?.ok){
          const target=game.lastPlay&&game.lastPlay.seat!==seat?game.lastPlay.pattern:null;
          const legal=core.generateCandidates(game.hands[seat],game.levelRank,target)
          ...
          if(legal.length)result=core.playCards(game,seat,legal[0].cards.map(c=>c.id));
          if(!result?.ok&&target)result=core.passTurn(game,seat);
          if(!result?.ok&&game.hands[seat]?.[0])result=core.playCards(game,seat,[game.hands[seat][0].id]);
        }
        if(!result?.ok){console.error('server AI turn failed',{seat,move,result});break;}
        sendState();
      }
    }finally{
      botRunning=false;
      if(game&&game.status==='playing'&&!seats[game.turn])setTimeout(()=>runBots(),60);
    }
  }
  ...
  function restartMatch(levelRank='2',difficulty='normal'){
    for(let i=0;i<4;i++)botDifficulty[i]=difficulty;
    const rank=core.RANKS.includes(levelRank)?levelRank:'2';
    game=core.createInitialGame({levelRank:rank,dealer:0,teamLevels:[rank,rank],levelTeam:0,round:1});
    chat.push({from:'系统',message:'房主已重新开局',at:Date.now(),clientId:'system',seat:null});
    if(chat.length>60)chat.shift();
    for(const x of seatedClients()){x.ready=true;updateSession(x);}
    sendState();broadcastRoom();broadcast({type:'chat',from:'系统',message:'房主已重新开局',at:Date.now(),clientId:'system',seat:null});runBots();
    return{ok:true};
  }
  ...
  wss.on('connection',ws=>{
    ...
    const id=Math.random().toString(36).slice(2,10);const c={id,ws,name:'玩家',seat:null,ready:false,joined:false,reconnectToken:null,kicked:false,intentionalLeave:false};clients.set(id,c);if(!hostClientId)hostClientId=id;
    ...
    ws.on('message',async raw=>{
      ...
      if(msg.type==='restartMatch'){
        if(id!==hostClientId)return safeSend(ws,{type:'error',message:'只有房主可以重新开局'});
        restartMatch(msg.levelRank||'2',msg.difficulty||'normal');return;
      }
      if(msg.type==='leave'){
        c.intentionalLeave=true;if(c.reconnectToken)sessions.delete(c.reconnectToken);releaseSeat(c);broadcastRoom();
        safeSend(ws,{type:'left'});setTimeout(()=>{try{ws.close();}catch{}},20);return;
      }
    ...
    ws.on('close',async()=>{
      ...
      if(!c.kicked&&!c.intentionalLeave)updateSession(c);else if(c.reconnectToken)sessions.delete(c.reconnectToken);
      clients.delete(id);
  ...
  if(advertise){
    beacon=dgram.createSocket({type:'udp4',reuseAddr:true});beacon.bind(()=>{
      try{beacon.setMulticastTTL(1);}catch{}
      try{beacon.setBroadcast(true);}catch{}
      const send=()=>{
        const payload=Buffer.from(JSON.stringify({type:'GUANDAN_ROOM',roomCode,roomName,port}));
        try{beacon.send(payload,0,payload.length,DISCOVERY_PORT,MULTICAST_ADDR,()=>{});}catch{}
        try{beacon.send(payload,0,payload.length,DISCOVERY_PORT,'255.255.255.255',()=>{});}catch{}
      };
      send();beaconTimer=setInterval(send,1200);
    });
  }
  return{
